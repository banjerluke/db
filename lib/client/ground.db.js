                ...
                .iterate((doc, id) => {
                  Kernel.defer(() => {
                    ...
                    if (!this._collection._docs._map[id]) {
                      if (this.decodeDocument) { doc = this.decodeDocument(doc); }
      ...
      this.storage
        .ready(() => {
          ...
          if (remove) {
            ...
            if (this.encodeDocument) { doc = this.encodeDocument(doc); }
            doc = EJSON.toJSONValue(doc);
            this.storage
              .getItem(doc._id, (err, savedDoc) => {
                if (err || !_.isEqual(savedDoc, doc)) {
                  this.storage
                    .setItem(doc._id, doc)
                    .then(() => {
                      this.pendingWrites.dec();
